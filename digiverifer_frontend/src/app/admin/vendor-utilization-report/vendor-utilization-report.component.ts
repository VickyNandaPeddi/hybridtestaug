  ...
  exportexcel(): void
    ...
    this.company.forEach((value: any=[], key: string) => {
      console.log("key",key)
        ...
        }else if (key.includes('PROCESSDECLINED')){
            // console.log("Outside",key, value.length, value)
    ...
    XLSX.utils.book_append_sheet(wb, ws_newupload, "CLEARED");
    XLSX.utils.book_append_sheet(wb, ws_REINVITE, "IN PROGRESS");
    XLSX.utils.book_append_sheet(wb, ws_FINALREPORT, "IN SUFFICIENCY");
    XLSX.utils.book_append_sheet(wb, ws_PENDINGNOW, "MAJOR DISCREPENCY");
    XLSX.utils.book_append_sheet(wb, ws_PENDINGAPPROVAL, "MINOR DISCREPENCY");
    XLSX.utils.book_append_sheet(wb, ws_INVITATIONEXPIRED, "UNABLE TO VERIFY");
      ...
      for (let item in this.getCustomerUtilizationReport){
        ...
        Object.keys(this.getCustomerUtilizationReport[item]).find((key)=>{
          if (key.includes('Code')){
            ...
            this.customers.getVendorDetailsByStatus(this.utilizationReportClick.value).subscribe((result: any)=>{
              if (result['data']['organizationName']!=null && result.data.candidateDetailsDto.length>0){
  ...
  onSubmitFilter(utilizationReportFilter:FormGroup){
    ...
    this.utilizationReportFilter.patchValue({
      ...
      organizationIds: organizationIds
        ...
        this.customers.postVendorUtilizationReport(this.utilizationReportFilter.value).subscribe((data:any)=>{
